...
OM_dot_sec = -3/2*n*J2*rE^2/(a^2*(1-e^2)^2)*cos(i);
om_dot_sec = 3/2*n*J2*rE^2/(a^2*(1-e^2)^2)*(2 - 5/2*sin(i)^2);

delta_OM_sec = -3*pi*J2*rE^2/(a^2*(1-e^2)^2)*cos(i);
delta_om_sec = 3*pi*J2*rE^2/(a^2*(1-e^2)^2)*(2 - 5/2*sin(i)^2);
...
% filter Long term
a_filter2 = smooth(tspan, S(:,1), 0.05, 'rlowess');
e_filter2 = smooth(tspan, S(:,2), 0.05, 'rloess');
i_filter2 = rad2deg(wrapToPi(smooth(tspan, S(:,3), 0.05, 'rloess')));
OM_filter2 = rad2deg(wrapTo2Pi(smooth(tspan, S(:,4), 0.05, 'rloess')));
om_filter2 = rad2deg(wrapTo2Pi(smooth(tspan, S(:,5), 0.05, 'rloess')));
th_filter2 = rad2deg(smooth(tspan, S(:,6), 0.05, 'rloess'));
...
plot( tspan, a_filter2, '-c', 'LineWidth', 2)
...
legend('Oscillation', 'Long Term', 'Secular', 'Secular effect drag')
...
plot( tspan, e_filter2, '-c', 'LineWidth', 2)
...
legend('Oscillation', 'Long Term', 'Secular')
...
plot( tspan, i_filter2, '-c', 'LineWidth', 2)
...
legend('Oscillation', 'Long Term', 'Secular', 'Secular effect J2')
%%
% om plot
figure
hold on;
...
% Find indexes 
index1 = find(tspan > 400, 1, 'first');
index2 = find(tspan > 420, 1, 'first');

ax2 = axes('position',[.15 .175 .25 .25]);
box on % put box around new pair of axes
hold on
plot( tspan(index1:index2),  rad2deg(S(index1:index2,5)), '-r', 'LineWidth', 1)
plot( tspan(index1:index2), om_filter2(index1:index2), '-c', 'LineWidth', 2.5)
plot( tspan(index1:index2), om_filter1(index1:index2), '-g', 'LineWidth', 2.5)
plot(400:420, rad2deg(kep0(1,5) + (400:420)*delta_om_sec), '-b', 'LineWidth', 2);
axis tight
...
plot( tspan, th_filter2, '-c', 'LineWidth', 2)
